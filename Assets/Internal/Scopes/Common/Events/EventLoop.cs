...
namespace Internal
{
    public class EventLoop : IEventLoop
    {
        ...
        public async UniTask RunConstruct(IReadOnlyLifetime lifetime)
        {
            ...
            await InvokeAsync(_baseSetupAsync, l => l.OnBaseSetupAsync(lifetime));

            Invoke(_setup, l => l.OnSetup(lifetime));
            await InvokeAsync(_setupAsync, l => l.OnSetupAsync(lifetime));

            Invoke(_setupCompletion, l => l.OnSetupCompletion(lifetime));
            await InvokeAsync(_setupCompletionAsync, l => l.OnSetupCompletionAsync(lifetime));
        }

        public async UniTask RunLoaded(IReadOnlyLifetime lifetime)
        {
            Invoke(_loaded, l => l.OnLoaded(lifetime));
            await InvokeAsync(_loadedAsync, l => l.OnLoadedAsync(lifetime));
        }

        public async UniTask RunDispose()
        {
            Invoke(_dispose, l => l.OnDispose());
            await InvokeAsync(_disposeAsync, l => l.OnDisposeAsync());
        }
